...
@pytest.mark.asyncio
async def test_hightest_priority_agents_get_provided_by_highest_ethics_score():
    """
     In this test case, agent a, agent b and agent c are negotiating for a solution over
     one interval. Agent d, agent e and agent f possess the needed flexibility for a, b and c.
...
@pytest.mark.asyncio
async def test_one_agent_provides_most_flexibility_and_has_to_prioritise():
    """
     In this test case, agent a, agent b and agent c are negotiating for a solution over
     one interval. Agent d, agent e and agent f possess the needed flexibility for a, b and c, but it is not
     distributed equally between them. Agent f possesses the most flexibility, meaning he is forced to prioritise
     requesting agents based on their ethics score. Agent d and agent provide the rest that agent f can't.
    ...
    agent_b.update_flexibility(t_start=first_interval, min_p=0, max_p=0)
    agent_c.update_flexibility(t_start=first_interval, min_p=0, max_p=0)
    agent_d.update_flexibility(t_start=first_interval, min_p=0, max_p=30)
    agent_e.update_flexibility(t_start=first_interval, min_p=0, max_p=30)
    agent_f.update_flexibility(t_start=first_interval, min_p=0, max_p=240)
