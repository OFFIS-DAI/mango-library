...
def test_cohda_init():
    cohda = COHDANegotiation(
        schedule_provider=lambda: [[0, 1, 2], [1, 2, 3]],
        is_local_acceptable=lambda s: True,
        part_id="1",
    )
    input_wm = WorkingMemory(
        target_params=([1, 2, 3], [1, 1, 1]),
...
def test_cohda_selection_multi():
    ...
    init_wm = WorkingMemory(
        ([1, 2, 1], [1, 1, 1]), SystemConfig({}), SolutionCandidate("1", {}, 0)
    )
...
@pytest.mark.parametrize(
    "old_sysconfig, old_candidate, wms, expected_sysconfig, expected_candidate",
...
def test_perceive(
    ...
    for part_id in new_sysconfig.schedule_choices:
        assert np.array_equal(
            new_sysconfig.schedule_choices[part_id].schedule,
            expected_sysconfig.schedule_choices[part_id].schedule,
        ), (
        ...
        assert np.array_equal(
            new_candidate.schedules[part_id], expected_candidate.schedules[part_id]
        )
...
async def test_optimize_simple_test_case():
    ...
    assert np.array_equal(
        cohda_negotiation._memory.solution_candidate.schedules["0"],
...
def test_schedule_provider_with_additional_parameters():
    ...
    init_wms = [
        WorkingMemory(
            ...
            solution_candidate=SolutionCandidate(
                schedules={"2": [0, 0, 2, 2, 2]}, agent_id="2", perf=-3
            ),
        )
    ]
    for schedule_provider in schedule_providers:
        cohda_negotiation = COHDANegotiation(
            ...
            is_local_acceptable=lambda _: True,
            part_id="1",
        )
