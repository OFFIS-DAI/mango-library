...
async def create_agents(
    ...
    for i in range(num_agents):
        ...
        cohda_role = MultiObjectiveCOHDARole(
            ...
            use_fixed_ref_point=use_fixed_ref_point,
            offsets=offsets,
        )
...
async def wait_for_coalition_built(agents):
    for agent in agents:
        while (
            len(agent._role_context.get_or_create_model(CoalitionModel)._assignments)
            < 1
        ):
