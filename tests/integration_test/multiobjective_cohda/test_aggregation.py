...
from mango_library.negotiation.multiobjective_cohda.data_classes import Target, SolutionCandidate, SolutionPoint
from mango_library.negotiation.multiobjective_cohda.mocohda_solution_aggregation import MoCohdaSolutionAggregationRole
...
from mango_library.negotiation.multiobjective_cohda.multiobjective_cohda import MultiObjectiveCOHDARole, \
    MoCohdaNegotiationModel, MoCohdaSolutionModel
...
async def test_coalition_to_mocohda_with_termination():
    ...
    termination_detector_role = NegotiationTerminationDetectorRole(aggregator_addr=c.addr,
                                                                   aggregator_id=controller_agent.aid)
    controller_agent.add_role(termination_detector_role)
    aggregation_role = MoCohdaSolutionAggregationRole(solution_point_choosing_function=choose_first_solution_point)
    ...
    for i in range(NUM_AGENTS):
        ...
        cohda_role = MultiObjectiveCOHDARole(
            ...
            pick_func=None, mutate_func=None)
    ...
    coalition_initiator_role = CoalitionInitiatorRole(addrs, 'mocohda', 'mocohda-negotiation')
    ...
    cohda_agents[0].add_role(
        MoCohdaNegotiationDirectStarterRole(target_params=None, num_solution_points=NUM_CANDIDATES))
    ...
    cohda_negotiation = \
        list(cohda_agents[0]._agent_context.get_or_create_model(MoCohdaNegotiationModel)._negotiations.values())[0]
    cohda_negotiation_id = \
        list(cohda_agents[0]._agent_context.get_or_create_model(MoCohdaNegotiationModel)._negotiations.keys())[0]
    cluster_schedule = cohda_negotiation._memory.solution_candidate.solution_points[0].cluster_schedule
    ...
    for index, agent in enumerate(cohda_agents):
        ...
        assert np.array_equal(get_final_schedule(agent),
        ...
        assert np.array_equal(get_final_schedule(agent),
                              solution_in_aggregator.cluster_schedule[solution_in_aggregator.idx[agent.aid[5:]]])
...
def get_final_schedule(cohda_agent):
    return list(cohda_agent._agent_context.get_or_create_model(MoCohdaSolutionModel)._final_schedules.values())[0]
